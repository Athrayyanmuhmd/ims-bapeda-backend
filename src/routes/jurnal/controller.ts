...
export const listJurnal = async (req: AuthRequest, res: Response) => {
  ...
  const result = await jurnalService.listJurnal({ skip, rows, orderKey, orderRule, searchFilters }, pesertaMagangId);
...
export const getJurnalDetail = async (req: AuthRequest, res: Response) => {
  const result = await jurnalService.getJurnalDetail(req.params.id as string);
  if (!result.ok) { fail(res, result.message, null, result.status); return; }
  ok(res, result.data);
};
...
export const createJurnal = async (req: AuthRequest, res: Response) => {
  ...
  const result = await jurnalService.createJurnal({ pesertaMagangId, tanggal, kegiatan });
  if (!result.ok) { fail(res, result.message, null, result.status); return; }
...
export const updateJurnal = async (req: AuthRequest, res: Response) => {
  ...
  const result = await jurnalService.updateJurnal(req.params.id as string, { tanggal, kegiatan });
  if (!result.ok) { fail(res, result.message, null, result.status); return; }
...
export const deleteJurnal = async (req: AuthRequest, res: Response) => {
  const result = await jurnalService.deleteJurnal(req.params.id as string);
  if (!result.ok) { fail(res, result.message, null, result.status); return; }
